...
\begin{equation}
  V(t|\theta) = -\hbar\sum_m g_m(\theta)
    ...
    + \hat{a}_m^\dagger\,\sigma^- e^{-i(\omega_{eg}-\omega_m)t}
    \Bigr),
  \label{eq:Vt}
\end{equation}
...
\begin{equation}
  ...
  \label{eq:gm}
\end{equation}
with effective mode volume $V_{\mathrm{eff}}^{(m)}(\theta) \sim
\pi a^2 L_{\mathrm{eff}}$.  For the dominant $\mathrm{LP}_{01}$
...
to a coherent amplitude $\mathcal{A}(x|\theta)$ that drives the
